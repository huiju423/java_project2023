...
public class FoodReservation extends JPanel {

	public FoodReservation(FoodDAO foodDao, String categoryName, int cNum) {
		// 기본 프레임 세팅
		...
		// 음식 설명 이미지
		JLabel imgFood = new JLabel(FoodHtmlUtils.imgHtmlParser(foodDao.Info().get(cNum).getImageUrl()));
		imgFood.setSize(545, 350);
		imgFood.setLocation(20, 20);
		add(imgFood);

		JLabel choiceName = new JLabel("<HTML><body>" + "☆ 장소 : " + foodDao.Info().get(cNum).getPlaceName() + "<br>"
				+ "☆ 음식 이름 : "
				+ foodDao.Info().get(cNum).getFoodName() + "<br>" + "☆ 주소 : " + foodDao.Info().get(cNum).getAddressName()
				+ "<br>" + "☆ 번호 : " + foodDao.Info().get(cNum).getPhone() + "</body></HTML>");
		...
		btnBack.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				FoodBase.getDispose();
				FoodBase.getInstance(new FoodRandomChoice(foodDao, categoryName, cNum));
			}
		});
